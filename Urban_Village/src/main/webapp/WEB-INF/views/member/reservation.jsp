...
.container {
   max-width: 1100px; /* 기존 900px에서 증가 */
   background: white;
   padding: 30px;
   border-radius: 10px;
...
.left-section {
   width: 60%; /* 기존 55%에서 증가 */
}

.right-section {
   width: 38%; /* 기존 40%에서 감소 */
   background: #f8f9fa;
   padding: 20px;
   border-radius: 10px;
}
   ...
   <div class="container">
      <div class="left-section">
         ...
         <div class="mb-3">
            ...
            <label for="checkout" class="form-label">체크아웃 날짜</label> <input
               type="date" class="form-control" id="checkout" value="2025-04-18">

            <label for="guests" class="form-label">게스트 수</label> <input
               type="number" class="form-control" id="guests" value="1" min="1">
         </div>
         ...
         <div class="text-center">
            <button type="button" class="btn btn-primary btn-lg"
               data-bs-toggle="modal" data-bs-target="#paymentModal">확인 및
               결제</button>
         </div>
               ...
               <div class="modal-content">
                  ...
                  <div class="modal-body">
                     <input type="checkbox" id="agreeTerms"> <label
                        ...
                        data-bs-toggle="modal" data-bs-target="#termsModal">[내용 확인]</a>
                     </label>
                  </div>
                  <div class="modal-footer">
      ...
      <div class="right-section">
         ...
         <div class="mb-3 total-price">
            <h5>총액</h5>
            <p>
               ₩<span id="total-price">1,709,836</span> (KRW)
            </p>
   ...
   <script>
      $(document).ready(function() {
         ...
         $('#termsModal .btn-primary').click(function() {
            $('#termsModal').modal('hide'); // 약관 모달 닫기

            setTimeout(function() {
               $('#agreeTerms').prop('checked', true).trigger('change'); // 체크박스 자동 체크 및 이벤트 발생
               $('#paymentModal').modal('show'); // 결제 확인 모달 다시 열기
            }, 500);
         });

         // 체크박스 상태 변경 감지하여 결제 확인 버튼 활성화
         $('#agreeTerms').on('change', function() {
            $('#confirmPayment').prop('disabled', !$(this).is(':checked'));
         });

         // 결제 확인 버튼 클릭 시 결제 완료 모달 표시
         $('#confirmPayment').click(function() {
            $('#paymentModal').modal('hide');
            setTimeout(function() {
               $('#successModal').modal('show');
