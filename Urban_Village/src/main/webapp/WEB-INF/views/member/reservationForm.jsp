...
<body>
    ...
    <form id="reservationForm" action="/Urban_Village/member/reservation.do" method="POST">
		...
		$(document)
				.ready(
						function() {
							...
							$('#paymentModal, #termsModal, #successModal').on(
							...
							// 가격 업데이트 이벤트 핸들러
							$('#checkin, #checkout, #guests').on('change',
									updatePrice);
							updatePrice();
						    ...
						    $('#reservationForm').on('submit', function (event) {
						        ...
						        copyValues(); // 폼 제출 전에 price 값을 hidden 필드에 복사
						        $('#paymentModal').modal('show'); // 결제 확인 모달 표시
						    });

							// 약관 확인 후 체크박스 자동 체크 및 결제 확인 창으로 이동
							$('#termsModal .btn-primary').click(
									function() {
